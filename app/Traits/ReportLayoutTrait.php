...
{
    // Retrieve the logo path directly from app settings
    $logoPath = app_setting()->logo; // This should be the path like 'application/your_logo.png'

    // Check if the logo path is not empty and the file exists in storage
    if ($logoPath && Storage::exists('public/' . $logoPath)) {
        return base64_encode(Storage::get('public/' . $logoPath)); // Get the logo from storage
    }

    // Fallback to a default logo if the uploaded logo doesn't exist
