...
class HomeController extends Controller
{
    ...
    public function myProfile()
    {
        ...
        $total_absent_days  = $total_working_days - $present_empl;
                     $hire_date = $employee_info->joining_date;

                $days_since_hire = (new \DateTime())->diff(new \DateTime($hire_date))->days;

        $absent_days = 0;
           if ($total_absent_days > 0) {
           // $absent_days = $total_absent_days;
           //$total_absent_days = max(0, $total_absent_days - $days_since_hire);
           $total_absent_days = $absent_days + $total_absent_days ;
           $absent_days = $days_since_hire -$present_empl;
