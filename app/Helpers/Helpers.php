...
if (!function_exists('app_setting')) {
    function app_setting()
    {
        ...
        // Adjust the paths for the logo and sidebar logo
        if (file_exists(public_path('picturee/' . $appSetting->logo))) {
            $appSetting->logo = asset('picturee/' . $appSetting->logo);
        } else {
            $appSetting->logo = asset('picturee/logonam.jpg'); // Default logo if not found
        }

        if (file_exists(public_path('picturee/' . $appSetting->sidebar_logo))) {
            $appSetting->sidebar_logo = asset('picturee/' . $appSetting->sidebar_logo);
        } else {
            $appSetting->sidebar_logo = asset('picturee/logonam.jpg'); // Default sidebar logo
        }

          $collapsedLogoPath = public_path('picturee/colap.png');
        if (file_exists($collapsedLogoPath)) {
            $appSetting->sidebar_collapsed_logo = asset('picturee/colap.png');
        } else {
            $appSetting->sidebar_collapsed_logo = asset('picturee/logonam.jpg'); // Default if not found
        }

        if (file_exists(public_path('picturee/' . $appSetting->favicon))) {
            $appSetting->favicon = asset('picturee/' . $appSetting->favicon);
        } else {
            $appSetting->favicon = asset('picturee/'); // Default favicon if not found
        }

        if (file_exists(public_path('picturee/' . $appSetting->login_image))) {
            $appSetting->login_image = asset('picturee/' . $appSetting->login_image);
        } else {
            $appSetting->login_image = asset('picturee/'); // Default login image if not found
        }
