...
$(document).ready(function () {
  ...
  function calculateTotalDays() {
    ...
    if (start && end) {
      ...
      // Check if start date is before or same as end date
      if (startDay <= endDay) {
        var millisecondsPerDay = 1000 * 60 * 60 * 24;
        var millisBetween = endDay.getTime() - startDay.getTime();
        var totalDays = Math.floor(millisBetween / millisecondsPerDay) ;

        // Set the calculated total days in the field
        $('#total_day').val(totalDays);
      } else {
        alert('Start date cannot be greater than end date.');
        $('#total_day').val(''); // Clear the total day field if invalid date range
      }
    }
  }
});
