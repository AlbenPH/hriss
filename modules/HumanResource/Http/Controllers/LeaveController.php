...
class LeaveController extends Controller
{
    ...
    public function approved(Request $request, ApplyLeave $leave)
    {
        ...
        $validated['leave_approved_date'] = Carbon::now(); // Ensure this line is included
        $validated['is_approved'] = 1; // Set approval status to true
        $validated['is_approved_by_manager'] = 1; // Ensure manager approval is set
    ...
    public function ApprovedByManager(Request $request, $uuid)
    {
        ...
        $request->validate([
            "leave_approved_start_date" => 'required|date',
            "leave_approved_end_date" => 'required|date',
            "total_approved_day" => 'required|numeric',
        ]);

        // Find the leave application by UUID
        $leave = ApplyLeave::where('uuid', $uuid)->firstOrFail();

        // Update the leave application with approved details
        $leave->is_approved_by_manager = 1; // Set manager approval
        $leave->is_approved = 1; // Ensure overall approval is set
        $leave->leave_approved_start_date = $request->leave_approved_start_date;
        $leave->leave_approved_end_date = $request->leave_approved_end_date;
        $leave->total_approved_day = $request->total_approved_day;
        $leave->manager_approved_description = $request->description;
        $leave->approved_by_manager = Auth::id(); // Set manager ID
        $leave->leave_approved_date = Carbon::now(); // Set leave approved date
        $leave->manager_approved_date = Carbon::now(); // Set the approval date to the current date and time
